"""
MSHSL (Minnesota State Association) DataSource Adapter

Provides authoritative tournament brackets, seeds, scores, and championship results
for Minnesota high school basketball.

**Data Authority**: MSHSL is the source of truth for:
...
- All regions of Minnesota

**Minnesota Basketball Context**:
- 500+ schools
- Notable tradition (Kevin McHale,Tyus Jones,Kris Humphries)
- MSHSL manages all high school athletics in Minnesota
...
**Recommended Use**: MSHSL provides official tournament brackets.
...
class MinnesotaMSHSLDataSource(AssociationAdapterBase):
    """
    Minnesota MSHSL adapter.
    ...
    **ARCHITECTURE**:
    ...
    - Generates unique game IDs: mshsl_{year}_{class}_{home}@{away}
    ...
    **LIMITATIONS**:
    ...
    source_type = DataSourceType.MSHSL
    source_name = "MSHSL"
    base_url = "https://www.mshsl.org"
    region = DataSourceRegion.US_MN

    # MSHSL specific constants
    ...
    def __init__(self):
        """Initialize MSHSL datasource with Minnesota-specific configuration."""
        super().__init__()
    ...
    def _build_bracket_url(
        ...
        **MSHSL URL Format**:
        ...
        # Try multiple URL patterns (MSHSL structure may vary)
    ...
    async def get_tournament_brackets(
        ...
        **ENUMERATION STRATEGY**:
        ...
        Args:
            ...
            classification: Specific classification (4A, 3A, 2A, 1A), None for all
            gender: "Boys" or "Girls"
        ...
        self.logger.info(
            f"Fetching MSHSL tournament brackets",
            year=year,
        ...
        self.logger.info(
            f"Fetched all MSHSL tournament brackets",
            year=year,
    ...
    def _parse_bracket_html(
        ...
        MSHSL bracket pages typically contain:
    ...
    def _create_game(
        ...
        team1_id = canonical_team_id("mshsl", team1)
        team2_id = canonical_team_id("mshsl", team2)

        return Game(
            game_id=f"mshsl_{year}_{classification.lower()}_{team1_id.split('_', 1)[1]}_vs_{team2_id.split('_', 1)[1]}",
    ...
    async def get_player(self, player_id: str) -> Optional[Player]:
        self.logger.warning("MSHSL does not provide player data - use MaxPreps for Minnesota")
        return None
