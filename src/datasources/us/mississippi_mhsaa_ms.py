"""
MHSAA_MS (Mississippi State Association) DataSource Adapter

Provides authoritative tournament brackets, seeds, scores, and championship results
for Mississippi high school basketball.

**Data Authority**: MHSAA_MS is the source of truth for:
...
**Base URL**: https://www.misshsaa.com
...
- All regions of Mississippi

**Mississippi Basketball Context**:
- 300+ schools
- Strong basketball tradition
- MHSAA_MS manages all high school athletics in Mississippi
...
**Recommended Use**: MHSAA_MS provides official tournament brackets.
...
class MississippiMHSAA_MSDataSource(AssociationAdapterBase):
    """
    Mississippi MHSAA_MS adapter.
    ...
    **ARCHITECTURE**:
    ...
    - Generates unique game IDs: mhsaa_ms_{year}_{class}_{home}@{away}
    ...
    **LIMITATIONS**:
    ...
    source_type = DataSourceType.MHSAA_MS
    source_name = "MHSAA_MS"
    base_url = "https://www.misshsaa.com"
    region = DataSourceRegion.US_MS

    # MHSAA_MS specific constants
    CLASSIFICATIONS = ["6A", "5A", "4A", "3A", "2A", "1A"]
    GENDERS = ["Boys", "Girls"]
    MIN_YEAR = 2015  # Historical data availability
    STATE_CODE = "MS"
    STATE_NAME = "Mississippi"
    ORGANIZATION = "MHSAA_MS"

    def __init__(self):
        """Initialize MHSAA_MS datasource with Mississippi-specific configuration."""
        super().__init__()
        self.settings = get_settings()
        self.logger.info(
            "MHSAA_MS initialized",
    ...
    def _build_bracket_url(
        ...
        **MHSAA_MS URL Format**:
        ...
        # Try multiple URL patterns (MHSAA_MS structure may vary)
    ...
    async def get_tournament_brackets(
        ...
        **ENUMERATION STRATEGY**:
        ...
        self.logger.info(
            f"Fetching MHSAA_MS tournament brackets",
            year=year,
        ...
        self.logger.info(
            f"Fetched all MHSAA_MS tournament brackets",
            year=year,
    ...
    def _parse_bracket_html(
        ...
        MHSAA_MS bracket pages typically contain:
    ...
    def _create_game(
        ...
        team1_id = canonical_team_id("mhsaa_ms", team1)
        team2_id = canonical_team_id("mhsaa_ms", team2)

        return Game(
            game_id=f"mhsaa_ms_{year}_{classification.lower()}_{team1_id.split('_', 1)[1]}_vs_{team2_id.split('_', 1)[1]}",
            ...
            league=f"MHSAA_MS {classification}",
            season=f"{year-1}-{str(year)[2:]}",
            gender=gender.lower(),
    ...
    async def get_player(self, player_id: str) -> Optional[Player]:
        self.logger.warning("MHSAA_MS does not provide player data - use MaxPreps for Mississippi")
        return None
