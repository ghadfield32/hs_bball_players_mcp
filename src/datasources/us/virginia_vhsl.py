"""
VHSL (Virginia State Association) DataSource Adapter

Provides authoritative tournament brackets, seeds, scores, and championship results
for Virginia high school basketball.

**Data Authority**: VHSL is the source of truth for:
- Tournament brackets (6 classifications: 6, 5, 4, 3, 2, 1)
- Seeds and matchups
...
**Coverage**:
- Classifications: 6, 5, 4, 3, 2, 1 (enrollment-based)
- 320+ member schools
- Boys and Girls tournaments
- All regions of Virginia

**Virginia Basketball Context**:
- 320+ schools
- Notable tradition (Allen Iverson,Alonzo Mourning,Grant Hill)
- VHSL manages all high school athletics in Virginia
- Enrollment-based classifications (6 largest, 1 smallest)
...
**Recommended Use**: VHSL provides official tournament brackets.
...
class VirginiaVHSLDataSource(AssociationAdapterBase):
    """
    Virginia VHSL adapter.

    **PRIMARY PURPOSE**: Authoritative tournament bracket and postseason results.

    **COVERAGE**: 320+ schools with basketball programs.

    This adapter provides:
    1. Tournament brackets for all classifications (6, 5, 4, 3, 2, 1)
    ...
    **ARCHITECTURE**:
    ...
    - Enumerates classifications: 6, 5, 4, 3, 2, 1
    - Generates unique game IDs: vhsl_{year}_{class}_{home}@{away}
    ...
    **LIMITATIONS**:
    ...
    def __init__(self):
        """Initialize VHSL datasource with Virginia-specific configuration."""
        super().__init__()
    ...
    def _build_bracket_url(
        ...
        **VHSL URL Format**:
        ...
        /sports/basketball/boys/brackets/2025/6/
        /sports/basketball/girls/brackets/2025/6/
        ...
        Args:
            classification: Classification name (6, 5, 4, 3, 2, 1)
        ...
        # Try multiple URL patterns (VHSL structure may vary)
    ...
    async def get_tournament_brackets(
        ...
        **ENUMERATION STRATEGY**:
        ...
        Args:
            ...
            classification: Specific classification (6, 5, 4, 3, 2, 1), None for all
            gender: "Boys" or "Girls"
        ...
        self.logger.info(
            f"Fetching VHSL tournament brackets",
            year=year,
        ...
        self.logger.info(
            f"Fetched all VHSL tournament brackets",
            year=year,
    ...
    def _parse_bracket_html(
        ...
        VHSL bracket pages typically contain:
    ...
    def _create_game(
        ...
        team1_id = canonical_team_id("vhsl", team1)
        team2_id = canonical_team_id("vhsl", team2)

        return Game(
            game_id=f"vhsl_{year}_{classification.lower()}_{team1_id.split('_', 1)[1]}_vs_{team2_id.split('_', 1)[1]}",
    ...
    async def _parse_html_data(self, html: str, season: str) -> Dict:
        year = self._extract_year(season)
        soup = parse_html(html)
        bracket_data = self._parse_bracket_html(soup, year, "6", "Boys", "")
        return {
    ...
    async def get_player(self, player_id: str) -> Optional[Player]:
        self.logger.warning("VHSL does not provide player data - use MaxPreps for Virginia")
        return None
