"""
PIAA (Pennsylvania Interscholastic Athletic Association) DataSource Adapter

Provides authoritative tournament brackets, seeds, scores, and championship results
for Pennsylvania high school basketball. Strong basketball state with 600+ schools.

**Data Authority**: PIAA is the source of truth for:
- Tournament brackets (6 classifications: 6A-1A)
- Seeds and matchups
- Game dates, times, locations
- Final scores and champions
- Historical tournament data

**Base URL**: https://www.piaa.org
...
- All regions of Pennsylvania
- Strong basketball tradition

**Pennsylvania Basketball Context**:
- Strong basketball state (600+ schools)
- Notable tradition (Kobe Bryant, Wilt Chamberlain, Maurice Cheeks)
- PIAA manages all high school athletics in Pennsylvania
- Enrollment-based classifications (6A largest, 1A smallest)
- District tournaments → state championships
- State championships held at Giant Center (Hershey) or similar venues

**Special Features**:
- Historical bracket data available
- District tournament structure (12 districts)
...
**Recommended Use**: PIAA provides official tournament brackets.
...
class PennsylvaniaPIAADataSource(AssociationAdapterBase):
    """
    Pennsylvania Interscholastic Athletic Association (PIAA) adapter.

    **PRIMARY PURPOSE**: Authoritative tournament bracket and postseason results.

    **STRONG BASKETBALL STATE**: 600+ schools with rich basketball tradition.
    ...
    **LIMITATIONS**:
    ...
    def __init__(self):
        """Initialize PIAA datasource with Pennsylvania-specific configuration."""
        super().__init__()
    ...
    def _build_bracket_url(
        ...
        **PIAA URL Format**:
        ...
        /sports/basketball/girls/brackets/2025/5a/
        ...
        # Try multiple URL patterns (PIAA structure may vary)
    ...
    async def get_tournament_brackets(
        ...
        **ENUMERATION STRATEGY**:
        ...
        Args:
            ...
            classification: Specific classification (6A-1A), None for all
            gender: "Boys" or "Girls"
        ...
        self.logger.info(
            f"Fetching PIAA tournament brackets",
            year=year,
        ...
        self.logger.info(
            f"Fetched all PIAA tournament brackets",
            year=year,
    ...
    def _create_game(
        ...
        team1_id = canonical_team_id("piaa", team1)
        team2_id = canonical_team_id("piaa", team2)

        return Game(
            game_id=f"piaa_{year}_{classification.lower()}_{team1_id.split('_', 1)[1]}_vs_{team2_id.split('_', 1)[1]}",
    ...
    async def get_player(self, player_id: str) -> Optional[Player]:
        self.logger.warning("PIAA does not provide player data - use MaxPreps for Pennsylvania")
        return None
