"""
WIAA_WA (Washington State Association) DataSource Adapter

Provides authoritative tournament brackets, seeds, scores, and championship results
for Washington high school basketball.

**Data Authority**: WIAA_WA is the source of truth for:
- Tournament brackets (6 classifications: 4A, 3A, 2A, 1A, 2B, 1B)
- Seeds and matchups
- Game dates, times, locations
- Final scores and champions
- Historical tournament data

**Base URL**: https://www.wiaa.com
...
**Coverage**:
- Classifications: 4A, 3A, 2A, 1A, 2B, 1B (enrollment-based)
- 400+ member schools
- Boys and Girls tournaments
- All regions of Washington

**Washington Basketball Context**:
- 400+ schools
- Notable tradition (Jason Terry,Brandon Roy,Nate Robinson)
- WIAA_WA manages all high school athletics in Washington
- Enrollment-based classifications (4A highest, 1B lowest)
...
**Recommended Use**: WIAA_WA provides official tournament brackets.
...
class WashingtonWIAADataSource(AssociationAdapterBase):
    """
    Washington WIAA adapter.
    ...
    **COVERAGE**: 400+ schools with basketball programs.
    ...
    1. Tournament brackets for all classifications (4A, 3A, 2A, 1A, 2B, 1B)
    ...
    **ARCHITECTURE**:
    ...
    - Enumerates classifications: 4A, 3A, 2A, 1A, 2B, 1B
    - Generates unique game IDs: wiaa_wa_{year}_{class}_{home}@{away}
    ...
    **LIMITATIONS**:
    ...
    source_type = DataSourceType.WIAA_WA
    source_name = "WIAA_WA"
    base_url = "https://www.wiaa.com"
    region = DataSourceRegion.US_WA

    # WIAA_WA specific constants
    CLASSIFICATIONS = ["4A", "3A", "2A", "1A", "2B", "1B"]
    GENDERS = ["Boys", "Girls"]
    MIN_YEAR = 2015  # Historical data availability
    STATE_CODE = "WA"
    STATE_NAME = "Washington"
    ORGANIZATION = "WIAA_WA"

    def __init__(self):
        """Initialize WIAA_WA datasource with Washington-specific configuration."""
        super().__init__()
    ...
    def _build_bracket_url(
        ...
        **WIAA_WA URL Format**:
        ...
        /sports/basketball/boys/brackets/2025/4a/
        /sports/basketball/girls/brackets/2025/4a/
        ...
        Args:
            classification: Classification name (4A, 3A, 2A, 1A, 2B, 1B)
        ...
        # Try multiple URL patterns (WIAA_WA structure may vary)
    ...
    async def get_tournament_brackets(
        ...
        **ENUMERATION STRATEGY**:
        ...
        Args:
            ...
            classification: Specific classification (4A, 3A, 2A, 1A, 2B, 1B), None for all
            gender: "Boys" or "Girls"
        ...
        self.logger.info(
            f"Fetching WIAA_WA tournament brackets",
            year=year,
        ...
        self.logger.info(
            f"Fetched all WIAA_WA tournament brackets",
            year=year,
    ...
    def _parse_bracket_html(
        ...
        WIAA_WA bracket pages typically contain:
    ...
    def _create_game(
        ...
        team1_id = canonical_team_id("wiaa_wa", team1)
        team2_id = canonical_team_id("wiaa_wa", team2)

        return Game(
            game_id=f"wiaa_wa_{year}_{classification.lower()}_{team1_id.split('_', 1)[1]}_vs_{team2_id.split('_', 1)[1]}",
            ...
            league=f"WIAA_WA {classification}",
            season=f"{year-1}-{str(year)[2:]}",
            gender=gender.lower(),
    ...
    async def get_player(self, player_id: str) -> Optional[Player]:
        self.logger.warning("WIAA_WA does not provide player data - use MaxPreps for Washington")
        return None
