"""
AIA (Arizona State Association) DataSource Adapter

Provides authoritative tournament brackets, seeds, scores, and championship results
for Arizona high school basketball.

**Data Authority**: AIA is the source of truth for:
...
**Base URL**: https://www.aiaonline.org
...
- All regions of Arizona

**Arizona Basketball Context**:
- 300+ schools
- Strong basketball tradition
- AIA manages all high school athletics in Arizona
...
**Recommended Use**: AIA provides official tournament brackets.
...
class ArizonaAIADataSource(AssociationAdapterBase):
    """
    Arizona AIA adapter.
    ...
    **ARCHITECTURE**:
    ...
    - Generates unique game IDs: aia_{year}_{class}_{home}@{away}
    ...
    **LIMITATIONS**:
    ...
    source_type = DataSourceType.AIA
    source_name = "AIA"
    base_url = "https://www.aiaonline.org"
    region = DataSourceRegion.US_AZ
    ...
    def __init__(self):
        """Initialize AIA datasource with Arizona-specific configuration."""
        super().__init__()
    ...
    def _build_bracket_url(
        ...
        **AIA URL Format**:
        ...
        # Try multiple URL patterns (AIA structure may vary)
    ...
    async def get_tournament_brackets(
        ...
        **ENUMERATION STRATEGY**:
        ...
        self.logger.info(
            f"Fetching AIA tournament brackets",
            year=year,
        ...
        self.logger.info(
            f"Fetched all AIA tournament brackets",
            year=year,
    ...
    def _parse_bracket_html(
        ...
        AIA bracket pages typically contain:
    ...
    def _create_game(
        ...
        team1_id = canonical_team_id("aia", team1)
        team2_id = canonical_team_id("aia", team2)

        return Game(
            game_id=f"aia_{year}_{classification.lower()}_{team1_id.split('_', 1)[1]}_vs_{team2_id.split('_', 1)[1]}",
    ...
    async def get_player(self, player_id: str) -> Optional[Player]:
        self.logger.warning("AIA does not provide player data - use MaxPreps for Arizona")
        return None
