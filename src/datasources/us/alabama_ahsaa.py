"""
AHSAA (Alabama State Association) DataSource Adapter

Provides authoritative tournament brackets, seeds, scores, and championship results
for Alabama high school basketball.

**Data Authority**: AHSAA is the source of truth for:
- Tournament brackets (7 classifications: 7A, 6A, 5A, 4A, 3A, 2A, 1A)
- Seeds and matchups
- Game dates, times, locations
- Final scores and champions
- Historical tournament data

**Base URL**: https://www.ahsaa.com
...
- All regions of Alabama

**Alabama Basketball Context**:
- 400+ schools
- Strong basketball tradition
- AHSAA manages all high school athletics in Alabama
- Enrollment-based classifications (7A highest, 1A lowest)
...
**Recommended Use**: AHSAA provides official tournament brackets.
...
class AlabamaAHSAADataSource(AssociationAdapterBase):
    """
    Alabama AHSAA adapter.
    ...
    **COVERAGE**: 400+ schools with basketball programs.
    ...
    1. Tournament brackets for all classifications (7A, 6A, 5A, 4A, 3A, 2A, 1A)
    ...
    **ARCHITECTURE**:
    ...
    - Generates unique game IDs: ahsaa_{year}_{class}_{home}@{away}
    ...
    **LIMITATIONS**:
    ...
    def __init__(self):
        """Initialize AHSAA datasource with Alabama-specific configuration."""
        super().__init__()
    ...
    def _build_bracket_url(
        ...
        **AHSAA URL Format**:
        ...
        /sports/basketball/girls/brackets/2025/7a/
        ...
        Args:
            classification: Classification name (7A, 6A, 5A, 4A, 3A, 2A, 1A)
        ...
        # Try multiple URL patterns (AHSAA structure may vary)
    ...
    async def get_tournament_brackets(
        ...
        **ENUMERATION STRATEGY**:
        ...
        Args:
            ...
            classification: Specific classification (7A, 6A, 5A, 4A, 3A, 2A, 1A), None for all
            gender: "Boys" or "Girls"
        ...
        self.logger.info(
            f"Fetching AHSAA tournament brackets",
            year=year,
        ...
        self.logger.info(
            f"Fetched all AHSAA tournament brackets",
            year=year,
    ...
    def _parse_bracket_html(
        ...
        AHSAA bracket pages typically contain:
    ...
    def _create_game(
        ...
        team1_id = canonical_team_id("ahsaa", team1)
        team2_id = canonical_team_id("ahsaa", team2)

        return Game(
            game_id=f"ahsaa_{year}_{classification.lower()}_{team1_id.split('_', 1)[1]}_vs_{team2_id.split('_', 1)[1]}",
    ...
    async def get_player(self, player_id: str) -> Optional[Player]:
        self.logger.warning("AHSAA does not provide player data - use MaxPreps for Alabama")
        return None
