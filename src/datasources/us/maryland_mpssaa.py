"""
MPSSAA (Maryland State Association) DataSource Adapter

Provides authoritative tournament brackets, seeds, scores, and championship results
for Maryland high school basketball.

**Data Authority**: MPSSAA is the source of truth for:
...
**Maryland Basketball Context**:
- 250+ schools
- Notable tradition (Kevin Durant,Carmelo Anthony,Adrian Dantley)
- MPSSAA manages all high school athletics in Maryland
...
**Recommended Use**: MPSSAA provides official tournament brackets.
...
class MarylandMPSSAADataSource(AssociationAdapterBase):
    """
    Maryland MPSSAA adapter.
    ...
    **ARCHITECTURE**:
    ...
    - Generates unique game IDs: mpssaa_{year}_{class}_{home}@{away}
    ...
    **LIMITATIONS**:
    ...
    def __init__(self):
        """Initialize MPSSAA datasource with Maryland-specific configuration."""
        super().__init__()
    ...
    def _build_bracket_url(
        ...
        **MPSSAA URL Format**:
        ...
        # Try multiple URL patterns (MPSSAA structure may vary)
    ...
    async def get_tournament_brackets(
        ...
        **ENUMERATION STRATEGY**:
        ...
        self.logger.info(
            f"Fetching MPSSAA tournament brackets",
            year=year,
        ...
        self.logger.info(
            f"Fetched all MPSSAA tournament brackets",
            year=year,
    ...
    def _parse_bracket_html(
        ...
        MPSSAA bracket pages typically contain:
    ...
    def _create_game(
        ...
        team1_id = canonical_team_id("mpssaa", team1)
        team2_id = canonical_team_id("mpssaa", team2)

        return Game(
            game_id=f"mpssaa_{year}_{classification.lower()}_{team1_id.split('_', 1)[1]}_vs_{team2_id.split('_', 1)[1]}",
            ...
            league=f"MPSSAA {classification}",
            season=f"{year-1}-{str(year)[2:]}",
            gender=gender.lower(),
    ...
    async def get_player(self, player_id: str) -> Optional[Player]:
        self.logger.warning("MPSSAA does not provide player data - use MaxPreps for Maryland")
        return None
