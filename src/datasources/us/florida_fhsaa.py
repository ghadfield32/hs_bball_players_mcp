...
for Florida high school basketball. This is the THIRD LARGEST state system in the US.

**Data Authority**: FHSAA is the source of truth for:
- Tournament brackets (9 classifications: 7A-1A + Metro/Suburban)
- Seeds and matchups
- Game dates, times, locations
- Final scores and champions
- Historical tournament data

**Base URL**: https://www.fhsaa.org
...
**Coverage**:
- Classifications: 7A, 6A, 5A, 4A, 3A, 2A, 1A + Metro/Suburban (9 total)
- 800+ member schools (third largest in US)
- Boys and Girls tournaments
- All regions of Florida

**Florida Basketball Context**:
- Third-largest state system in US (800+ schools)
- Strong basketball tradition (Vince Carter, Tracy McGrady, Udonis Haslem)
- FHSAA manages all high school athletics in Florida
- Enrollment-based classifications (7A largest schools, 1A smallest)
- Metro/Suburban divisions for specific regions
- Regional tournaments → state championships

**Special Features**:
- **9 classifications** (most divisions of any state)
- Metro and Suburban divisions (urban/suburban split)
- Historical bracket data available
- State championships held at RP Funding Center (Lakeland)
...
**Recommended Use**: FHSAA provides official tournament brackets.
...
class FloridaFHSAADataSource(AssociationAdapterBase):
    """
    Florida High School Athletic Association (FHSAA) adapter.

    **PRIMARY PURPOSE**: Authoritative tournament bracket and postseason results.

    **THIRD LARGEST**: 800+ schools - largest in southeastern US.

    This adapter provides:
    1. Tournament brackets for all classifications (7A-1A + Metro/Suburban)
    ...
    **ARCHITECTURE**:
    ...
    - Enumerates classifications: 7A, 6A, 5A, 4A, 3A, 2A, 1A, Metro, Suburban
    - Generates unique game IDs: fhsaa_{year}_{class}_{home}@{away}
    ...
    **LIMITATIONS**:
    ...
    region = DataSourceRegion.US_FL

    # FHSAA specific constants
    # Florida has MORE classifications than most states (9 total)
    CLASSIFICATIONS = ["7A", "6A", "5A", "4A", "3A", "2A", "1A", "Metro", "Suburban"]
    GENDERS = ["Boys", "Girls"]
    ...
    def __init__(self):
        """Initialize FHSAA datasource with Florida-specific configuration."""
        super().__init__()
    ...
    def _build_bracket_url(
        ...
        **FHSAA URL Format**:
        ...
        /sports/basketball/boys/brackets/2025/7a/
        /sports/basketball/girls/brackets/2025/6a/

        Alternative patterns:
        /playoffs/{year}/basketball-{gender}-{classification}/
        /brackets/{year}-{sport}-{classification}/
        ```

        Args:
            classification: Classification name (7A-1A, Metro, Suburban)
        ...
        # Try multiple URL patterns (FHSAA structure may vary)
    ...
    async def get_tournament_brackets(
        ...
        **ENUMERATION STRATEGY**:
        ...
        - Otherwise: fetch current year, all classifications (9 total!)

        Args:
            season: Season string (e.g., "2024-25"), None for current
            classification: Specific classification (7A-1A, Metro, Suburban), None for all
            gender: "Boys" or "Girls"
        ...
        self.logger.info(
            f"Fetching FHSAA tournament brackets",
            year=year,
            classifications=classifications,
            gender=gender,
        )

        # Enumerate all bracket combinations (up to 9 classifications!)
        ...
        self.logger.info(
            f"Fetched all FHSAA tournament brackets",
            year=year,
    ...
    def _parse_bracket_html(
        ...
        FHSAA bracket pages typically contain:
    ...
    def _create_game(
        ...
        team1_id = canonical_team_id("fhsaa", team1)
        team2_id = canonical_team_id("fhsaa", team2)

        return Game(
            game_id=f"fhsaa_{year}_{classification.lower()}_{team1_id.split('_', 1)[1]}_vs_{team2_id.split('_', 1)[1]}",
    ...
    async def get_player(self, player_id: str) -> Optional[Player]:
        self.logger.warning("FHSAA does not provide player data - use MaxPreps for Florida")
        return None
