"""
NCHSAA (North Carolina State Association) DataSource Adapter

Provides authoritative tournament brackets, seeds, scores, and championship results
for North Carolina high school basketball.

**Data Authority**: NCHSAA is the source of truth for:
...
**North Carolina Basketball Context**:
- 400+ schools
- Notable tradition (Michael Jordan,Chris Paul,James Worthy)
- NCHSAA manages all high school athletics in North Carolina
...
**Recommended Use**: NCHSAA provides official tournament brackets.
...
class NCHSAADataSource(AssociationAdapterBase):
    ...
    **ARCHITECTURE**:
    ...
    - Generates unique game IDs: nchsaa_{year}_{class}_{home}@{away}
    ...
    **LIMITATIONS**:
    ...
    def __init__(self):
        """Initialize NCHSAA datasource with North Carolina-specific configuration."""
        super().__init__()
    ...
    def _build_bracket_url(
        ...
        **NCHSAA URL Format**:
        ...
        # Try multiple URL patterns (NCHSAA structure may vary)
    ...
    async def get_tournament_brackets(
        ...
        **ENUMERATION STRATEGY**:
        ...
        self.logger.info(
            f"Fetching NCHSAA tournament brackets",
            year=year,
        ...
        self.logger.info(
            f"Fetched all NCHSAA tournament brackets",
            year=year,
    ...
    def _parse_bracket_html(
        ...
        NCHSAA bracket pages typically contain:
    ...
    def _create_game(
        ...
        team1_id = canonical_team_id("nchsaa", team1)
        team2_id = canonical_team_id("nchsaa", team2)

        return Game(
            game_id=f"nchsaa_{year}_{classification.lower()}_{team1_id.split('_', 1)[1]}_vs_{team2_id.split('_', 1)[1]}",
            ...
            league=f"NCHSAA {classification}",
            season=f"{year-1}-{str(year)[2:]}",
            gender=gender.lower(),
    ...
    async def get_player(self, player_id: str) -> Optional[Player]:
        self.logger.warning("NCHSAA does not provide player data - use MaxPreps for North Carolina")
        return None
