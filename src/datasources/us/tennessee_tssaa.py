"""
TSSAA (Tennessee State Association) DataSource Adapter

Provides authoritative tournament brackets, seeds, scores, and championship results
for Tennessee high school basketball.

**Data Authority**: TSSAA is the source of truth for:
...
**Tennessee Basketball Context**:
- 400+ schools
- Strong basketball tradition
- TSSAA manages all high school athletics in Tennessee
...
**Recommended Use**: TSSAA provides official tournament brackets.
...
class TennesseeTSSAADataSource(AssociationAdapterBase):
    """
    Tennessee TSSAA adapter.
    ...
    **ARCHITECTURE**:
    ...
    - Generates unique game IDs: tssaa_{year}_{class}_{home}@{away}
    ...
    **LIMITATIONS**:
    ...
    source_name = "TSSAA"
    base_url = "https://www.tssaa.org"
    region = DataSourceRegion.US_TN

    # TSSAA specific constants
    ...
    def __init__(self):
        """Initialize TSSAA datasource with Tennessee-specific configuration."""
        super().__init__()
    ...
    def _build_bracket_url(
        ...
        **TSSAA URL Format**:
        ...
        # Try multiple URL patterns (TSSAA structure may vary)
    ...
    async def get_tournament_brackets(
        ...
        **ENUMERATION STRATEGY**:
        ...
        self.logger.info(
            f"Fetching TSSAA tournament brackets",
            year=year,
        ...
        self.logger.info(
            f"Fetched all TSSAA tournament brackets",
            year=year,
    ...
    def _parse_bracket_html(
        ...
        TSSAA bracket pages typically contain:
    ...
    def _create_game(
        ...
        team1_id = canonical_team_id("tssaa", team1)
        team2_id = canonical_team_id("tssaa", team2)

        return Game(
            game_id=f"tssaa_{year}_{classification.lower()}_{team1_id.split('_', 1)[1]}_vs_{team2_id.split('_', 1)[1]}",
    ...
    async def get_player(self, player_id: str) -> Optional[Player]:
        self.logger.warning("TSSAA does not provide player data - use MaxPreps for Tennessee")
        return None
