"""
KSHSAA (Kansas State Association) DataSource Adapter

Provides authoritative tournament brackets, seeds, scores, and championship results
for Kansas high school basketball.

**Data Authority**: KSHSAA is the source of truth for:
...
- All regions of Kansas

**Kansas Basketball Context**:
- 350+ schools
- Strong basketball tradition
- KSHSAA manages all high school athletics in Kansas
...
**Recommended Use**: KSHSAA provides official tournament brackets.
...
class KansasKSHSAADataSource(AssociationAdapterBase):
    """
    Kansas KSHSAA adapter.
    ...
    **ARCHITECTURE**:
    ...
    - Generates unique game IDs: kshsaa_{year}_{class}_{home}@{away}
    ...
    **LIMITATIONS**:
    ...
    def __init__(self):
        """Initialize KSHSAA datasource with Kansas-specific configuration."""
        super().__init__()
    ...
    def _build_bracket_url(
        ...
        **KSHSAA URL Format**:
        ...
        # Try multiple URL patterns (KSHSAA structure may vary)
    ...
    async def get_tournament_brackets(
        ...
        **ENUMERATION STRATEGY**:
        ...
        self.logger.info(
            f"Fetching KSHSAA tournament brackets",
            year=year,
        ...
        self.logger.info(
            f"Fetched all KSHSAA tournament brackets",
            year=year,
    ...
    def _parse_bracket_html(
        ...
        KSHSAA bracket pages typically contain:
    ...
    def _create_game(
        ...
        team1_id = canonical_team_id("kshsaa", team1)
        team2_id = canonical_team_id("kshsaa", team2)

        return Game(
            game_id=f"kshsaa_{year}_{classification.lower()}_{team1_id.split('_', 1)[1]}_vs_{team2_id.split('_', 1)[1]}",
            ...
            league=f"KSHSAA {classification}",
            season=f"{year-1}-{str(year)[2:]}",
            gender=gender.lower(),
    ...
    async def get_player(self, player_id: str) -> Optional[Player]:
        self.logger.warning("KSHSAA does not provide player data - use MaxPreps for Kansas")
        return None
