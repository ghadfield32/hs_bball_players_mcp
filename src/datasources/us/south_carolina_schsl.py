"""
SCHSL (South Carolina State Association) DataSource Adapter

Provides authoritative tournament brackets, seeds, scores, and championship results
for South Carolina high school basketball.

**Data Authority**: SCHSL is the source of truth for:
- Tournament brackets (5 classifications: 5A, 4A, 3A, 2A, 1A)
- Seeds and matchups
- Game dates, times, locations
- Final scores and champions
- Historical tournament data

**Base URL**: https://www.schsl.org
...
**Coverage**:
- Classifications: 5A, 4A, 3A, 2A, 1A (enrollment-based)
- 200+ member schools
- Boys and Girls tournaments
- All regions of South Carolina

**South Carolina Basketball Context**:
- 200+ schools
- Strong basketball tradition
- SCHSL manages all high school athletics in South Carolina
- Enrollment-based classifications (5A highest, 1A lowest)
...
**Recommended Use**: SCHSL provides official tournament brackets.
...
class SouthCarolinaSCHSLDataSource(AssociationAdapterBase):
    """
    South Carolina SCHSL adapter.

    **PRIMARY PURPOSE**: Authoritative tournament bracket and postseason results.

    **COVERAGE**: 200+ schools with basketball programs.

    This adapter provides:
    1. Tournament brackets for all classifications (5A, 4A, 3A, 2A, 1A)
    ...
    **ARCHITECTURE**:
    ...
    - Enumerates classifications: 5A, 4A, 3A, 2A, 1A
    - Generates unique game IDs: schsl_{year}_{class}_{home}@{away}
    ...
    **LIMITATIONS**:
    ...
    source_name = "SCHSL"
    base_url = "https://www.schsl.org"
    region = DataSourceRegion.US_SC

    # SCHSL specific constants
    ...
    def __init__(self):
        """Initialize SCHSL datasource with South Carolina-specific configuration."""
        super().__init__()
    ...
    def _build_bracket_url(
        ...
        **SCHSL URL Format**:
        ...
        /sports/basketball/boys/brackets/2025/5a/
        ...
        Args:
            classification: Classification name (5A, 4A, 3A, 2A, 1A)
        ...
        # Try multiple URL patterns (SCHSL structure may vary)
    ...
    async def get_tournament_brackets(
        ...
        **ENUMERATION STRATEGY**:
        ...
        Args:
            ...
            classification: Specific classification (5A, 4A, 3A, 2A, 1A), None for all
            gender: "Boys" or "Girls"
        ...
        self.logger.info(
            f"Fetching SCHSL tournament brackets",
            year=year,
        ...
        self.logger.info(
            f"Fetched all SCHSL tournament brackets",
            year=year,
    ...
    def _parse_bracket_html(
        ...
        SCHSL bracket pages typically contain:
    ...
    def _create_game(
        ...
        team1_id = canonical_team_id("schsl", team1)
        team2_id = canonical_team_id("schsl", team2)

        return Game(
            game_id=f"schsl_{year}_{classification.lower()}_{team1_id.split('_', 1)[1]}_vs_{team2_id.split('_', 1)[1]}",
    ...
    async def _parse_html_data(self, html: str, season: str) -> Dict:
        year = self._extract_year(season)
        soup = parse_html(html)
        bracket_data = self._parse_bracket_html(soup, year, "5A", "Boys", "")
        return {
    ...
    async def get_player(self, player_id: str) -> Optional[Player]:
        self.logger.warning("SCHSL does not provide player data - use MaxPreps for South Carolina")
        return None
