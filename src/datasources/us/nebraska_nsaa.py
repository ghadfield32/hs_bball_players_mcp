"""
NSAA (Nebraska State Association) DataSource Adapter

Provides authoritative tournament brackets, seeds, scores, and championship results
for Nebraska high school basketball.

**Data Authority**: NSAA is the source of truth for:
- Tournament brackets (6 classifications: A, B, C1, C2, D1, D2)
- Seeds and matchups
- Game dates, times, locations
- Final scores and champions
- Historical tournament data

**Base URL**: https://www.nsaahome.org
...
**Coverage**:
- Classifications: A, B, C1, C2, D1, D2 (enrollment-based)
- 250+ member schools
- Boys and Girls tournaments
- All regions of Nebraska

**Nebraska Basketball Context**:
- 250+ schools
- Strong basketball tradition
- NSAA manages all high school athletics in Nebraska
- Enrollment-based classifications (A highest, D2 lowest)
...
**Recommended Use**: NSAA provides official tournament brackets.
...
class NebraskaNSAADataSource(AssociationAdapterBase):
    """
    Nebraska NSAA adapter.

    **PRIMARY PURPOSE**: Authoritative tournament bracket and postseason results.

    **COVERAGE**: 250+ schools with basketball programs.

    This adapter provides:
    1. Tournament brackets for all classifications (A, B, C1, C2, D1, D2)
    ...
    **ARCHITECTURE**:
    ...
    - Enumerates classifications: A, B, C1, C2, D1, D2
    - Generates unique game IDs: nsaa_{year}_{class}_{home}@{away}
    ...
    **LIMITATIONS**:
    ...
    # NSAA specific constants
    CLASSIFICATIONS = ["A", "B", "C1", "C2", "D1", "D2"]
    GENDERS = ["Boys", "Girls"]
    ...
    def __init__(self):
        """Initialize NSAA datasource with Nebraska-specific configuration."""
        super().__init__()
    ...
    def _build_bracket_url(
        ...
        **NSAA URL Format**:
        ...
        /sports/basketball/boys/brackets/2025/a/
        ...
        Args:
            classification: Classification name (A, B, C1, C2, D1, D2)
        ...
        # Try multiple URL patterns (NSAA structure may vary)
    ...
    async def get_tournament_brackets(
        ...
        **ENUMERATION STRATEGY**:
        ...
        Args:
            ...
            classification: Specific classification (A, B, C1, C2, D1, D2), None for all
            gender: "Boys" or "Girls"
        ...
        self.logger.info(
            f"Fetching NSAA tournament brackets",
            year=year,
        ...
        self.logger.info(
            f"Fetched all NSAA tournament brackets",
            year=year,
    ...
    def _parse_bracket_html(
        ...
        NSAA bracket pages typically contain:
    ...
    def _create_game(
        ...
        team1_id = canonical_team_id("nsaa", team1)
        team2_id = canonical_team_id("nsaa", team2)

        return Game(
            game_id=f"nsaa_{year}_{classification.lower()}_{team1_id.split('_', 1)[1]}_vs_{team2_id.split('_', 1)[1]}",
    ...
    async def _parse_html_data(self, html: str, season: str) -> Dict:
        year = self._extract_year(season)
        soup = parse_html(html)
        bracket_data = self._parse_bracket_html(soup, year, "A", "Boys", "")
        return {
    ...
    async def get_player(self, player_id: str) -> Optional[Player]:
        self.logger.warning("NSAA does not provide player data - use MaxPreps for Nebraska")
        return None
