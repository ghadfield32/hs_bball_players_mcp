"""
Montana High School Association DataSource Adapter

Provides tournament brackets, schedules, and results for Montana high school basketball.

Base URL: https://www.mhsa.org
"""
...
class MontanaMhsaDataSource(AssociationAdapterBase):
    """
    Montana High School Association data source adapter.
    ...
    source_type = DataSourceType.MONTANA_MHSA
    source_name = "Montana MHSA"
    base_url = "https://www.mhsa.org"
    region = DataSourceRegion.US_MT

    def _get_season_url(self, season: str) -> str:
        """Get URL for Montana basketball season data."""
    ...
    async def _parse_json_data(self, json_data: Dict[str, Any], season: str) -> Dict[str, Any]:
        """
        Parse JSON data from Montana MHSA.
        ...
        # TODO: Implement based on actual JSON structure from Montana MHSA
        ...
        self.logger.info(
            f"Parsed Montana MHSA JSON data",
            season=season,
            teams=len(teams),
            games=len(games),
        )
    ...
    async def _parse_html_data(self, html: str, season: str) -> Dict[str, Any]:
        """
        Parse HTML data from Montana MHSA.
        ...
        self.logger.info(
            f"Parsed Montana MHSA HTML data",
            season=season,
            teams=len(teams),
            games=len(games),
        )
    ...
    def _parse_team_from_json(self, data: Dict[str, Any], season: str) -> Optional[Team]:
        ...
        try:
            ...
            team_id = f"montana_mhsa_{team_name.lower().replace(' ', '_')}"
    ...
    def _parse_game_from_json(self, data: Dict[str, Any], season: str) -> Optional[Game]:
        ...
        try:
            ...
            game_id = f"montana_mhsa_{data.get('id', data.get('game_id', hash(str(data))))}"
            ...
            return Game(
                ...
                home_team_id=f"montana_mhsa_{home_team.lower().replace(' ', '_')}",
                away_team_id=f"montana_mhsa_{away_team.lower().replace(' ', '_')}",
                game_date=game_date,
    ...
    def _parse_game_from_row(self, cells: List[Any], season: str) -> Optional[Game]:
        ...
        try:
            ...
            game_id = f"montana_mhsa_{home_team.lower().replace(' ', '_')}_{away_team.lower().replace(' ', '_')}"
