"""
LHSAA (Louisiana State Association) DataSource Adapter

Provides authoritative tournament brackets, seeds, scores, and championship results
for Louisiana high school basketball.

**Data Authority**: LHSAA is the source of truth for:
...
**Louisiana Basketball Context**:
- 400+ schools
- Strong basketball tradition
- LHSAA manages all high school athletics in Louisiana
...
**Recommended Use**: LHSAA provides official tournament brackets.
...
class LouisianaLHSAADataSource(AssociationAdapterBase):
    """
    Louisiana LHSAA adapter.
    ...
    **ARCHITECTURE**:
    ...
    - Generates unique game IDs: lhsaa_{year}_{class}_{home}@{away}
    ...
    **LIMITATIONS**:
    ...
    def __init__(self):
        """Initialize LHSAA datasource with Louisiana-specific configuration."""
        super().__init__()
    ...
    def _build_bracket_url(
        ...
        **LHSAA URL Format**:
        ...
        # Try multiple URL patterns (LHSAA structure may vary)
    ...
    async def get_tournament_brackets(
        ...
        **ENUMERATION STRATEGY**:
        ...
        self.logger.info(
            f"Fetching LHSAA tournament brackets",
            year=year,
        ...
        self.logger.info(
            f"Fetched all LHSAA tournament brackets",
            year=year,
    ...
    def _parse_bracket_html(
        ...
        LHSAA bracket pages typically contain:
    ...
    def _create_game(
        ...
        team1_id = canonical_team_id("lhsaa", team1)
        team2_id = canonical_team_id("lhsaa", team2)

        return Game(
            game_id=f"lhsaa_{year}_{classification.lower()}_{team1_id.split('_', 1)[1]}_vs_{team2_id.split('_', 1)[1]}",
    ...
    async def get_player(self, player_id: str) -> Optional[Player]:
        self.logger.warning("LHSAA does not provide player data - use MaxPreps for Louisiana")
        return None
