"""
MIAA_MA (Massachusetts State Association) DataSource Adapter

Provides authoritative tournament brackets, seeds, scores, and championship results
for Massachusetts high school basketball.

**Data Authority**: MIAA_MA is the source of truth for:
...
**Massachusetts Basketball Context**:
- 370+ schools
- Notable tradition (Patrick Ewing,Dana Barros,Michael Adams)
- MIAA_MA manages all high school athletics in Massachusetts
...
**Recommended Use**: MIAA_MA provides official tournament brackets.
...
class MassachusettsMiaaDataSource(AssociationAdapterBase):
    """
    Massachusetts MIAA adapter.

    **PRIMARY PURPOSE**: Authoritative tournament bracket and postseason results.

    **COVERAGE**: 370+ schools with basketball programs.
    ...
    **ARCHITECTURE**:
    ...
    - Generates unique game IDs: miaa_ma_{year}_{class}_{home}@{away}
    ...
    **LIMITATIONS**:
    ...
    def __init__(self):
        """Initialize MIAA datasource with Massachusetts-specific configuration."""
        super().__init__()
    ...
    def _build_bracket_url(
        ...
        **MIAA_MA URL Format**:
        ...
        /sports/basketball/boys/brackets/2025/division 1/
        /sports/basketball/girls/brackets/2025/division 1/
        ...
        # Try multiple URL patterns (MIAA_MA structure may vary)
    ...
    async def get_tournament_brackets(
        ...
        **ENUMERATION STRATEGY**:
        ...
        self.logger.info(
            f"Fetching MIAA_MA tournament brackets",
            year=year,
        ...
        self.logger.info(
            f"Fetched all MIAA_MA tournament brackets",
            year=year,
    ...
    def _parse_bracket_html(
        ...
        MIAA_MA bracket pages typically contain:
    ...
    def _create_game(
        ...
        team1_id = canonical_team_id("miaa", team1)
        team2_id = canonical_team_id("miaa", team2)

        return Game(
            game_id=f"miaa_{year}_{classification.lower()}_{team1_id.split('_', 1)[1]}_vs_{team2_id.split('_', 1)[1]}",
            ...
            league=f"MIAA_MA {classification}",
            season=f"{year-1}-{str(year)[2:]}",
            gender=gender.lower(),
    ...
    async def get_player(self, player_id: str) -> Optional[Player]:
        self.logger.warning("MIAA_MA does not provide player data - use MaxPreps for Massachusetts")
        return None
