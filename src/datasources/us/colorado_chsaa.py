"""
CHSAA (Colorado State Association) DataSource Adapter

Provides authoritative tournament brackets, seeds, scores, and championship results
for Colorado high school basketball.

**Data Authority**: CHSAA is the source of truth for:
...
**Colorado Basketball Context**:
- 350+ schools
- Strong basketball tradition
- CHSAA manages all high school athletics in Colorado
...
**Recommended Use**: CHSAA provides official tournament brackets.
...
class ColoradoCHSAADataSource(AssociationAdapterBase):
    """
    Colorado CHSAA adapter.

    **PRIMARY PURPOSE**: Authoritative tournament bracket and postseason results.

    **COVERAGE**: 350+ schools with basketball programs.
    ...
    **ARCHITECTURE**:
    ...
    - Generates unique game IDs: chsaa_{year}_{class}_{home}@{away}
    ...
    **LIMITATIONS**:
    ...
    def __init__(self):
        """Initialize CHSAA datasource with Colorado-specific configuration."""
        super().__init__()
    ...
    def _build_bracket_url(
        ...
        **CHSAA URL Format**:
        ...
        # Try multiple URL patterns (CHSAA structure may vary)
    ...
    async def get_tournament_brackets(
        ...
        **ENUMERATION STRATEGY**:
        ...
        self.logger.info(
            f"Fetching CHSAA tournament brackets",
            year=year,
        ...
        self.logger.info(
            f"Fetched all CHSAA tournament brackets",
            year=year,
    ...
    def _parse_bracket_html(
        ...
        CHSAA bracket pages typically contain:
    ...
    def _create_game(
        ...
        team1_id = canonical_team_id("chsaa", team1)
        team2_id = canonical_team_id("chsaa", team2)

        return Game(
            game_id=f"chsaa_{year}_{classification.lower()}_{team1_id.split('_', 1)[1]}_vs_{team2_id.split('_', 1)[1]}",
    ...
    async def get_player(self, player_id: str) -> Optional[Player]:
        self.logger.warning("CHSAA does not provide player data - use MaxPreps for Colorado")
        return None
