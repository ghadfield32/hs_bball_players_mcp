...
for Georgia high school basketball. Strong basketball state with 500+ schools.

**Data Authority**: GHSA is the source of truth for:
- Tournament brackets (7 classifications: 7A-1A)
- Seeds and matchups
...
Playoffs: /playoffs/{year}/basketball/{classification}
Brackets: /sports-media/basketball/{year}/playoffs
```

**Coverage**:
- Classifications: 7A, 6A, 5A, 4A, 3A, 2A, 1A (enrollment-based)
- 500+ member schools
- Boys and Girls tournaments
- All regions of Georgia
- Strong basketball tradition

**Georgia Basketball Context**:
- Strong basketball state (500+ schools)
- Notable tradition (Walt Frazier, Dwight Howard, Anthony Edwards)
- GHSA manages all high school athletics in Georgia
- Enrollment-based classifications (7A largest, 1A smallest)
- Regional tournaments → state championships
- State championships held at Georgia's "Macon Coliseum" or similar venues

**Special Features**:
- Historical bracket data available
- Regional structure (4-8 regions per classification)
...
**Recommended Use**: GHSA provides official tournament brackets.
...
class GeorgiaGHSADataSource(AssociationAdapterBase):
    """
    Georgia High School Association (GHSA) adapter.

    **PRIMARY PURPOSE**: Authoritative tournament bracket and postseason results.

    **STRONG BASKETBALL STATE**: 500+ schools with rich basketball tradition.

    This adapter provides:
    1. Tournament brackets for all classifications (7A-1A)
    ...
    **ARCHITECTURE**:
    ...
    - Enumerates classifications: 7A, 6A, 5A, 4A, 3A, 2A, 1A
    - Generates unique game IDs: ghsa_{year}_{class}_{home}@{away}
    ...
    **LIMITATIONS**:
    ...
    def __init__(self):
        """Initialize GHSA datasource with Georgia-specific configuration."""
        super().__init__()
    ...
    def _build_bracket_url(
        ...
        """
        Build URL for GHSA tournament bracket.

        **GHSA URL Format** (verified 2024-11-13):
        ```
        /{season}-ghsa-class-{classification}-{gender}-state-basketball-championship-bracket

        Examples:
        /2023-2024-ghsa-class-aaaaa-boys-state-basketball-championship-bracket
        /2023-2024-ghsa-class-aaaaaaa-girls-state-basketball-championship-bracket
        ```

        **Important**: GHSA uses "A" notation (AAAAA = 5A), not numeric "5A".

        Args:
            classification: Classification name (7A-1A)
        ...
        # Convert numeric classification to A notation: 7A → AAAAAAA (7 A's)
        if classification.upper().endswith("A"):
            num = int(classification.upper()[:-1])  # Extract number from "7A"
            class_a_notation = "A" * num  # "7A" → "AAAAAAA"
        else:
            class_a_notation = classification.upper()

        class_lower = class_a_notation.lower()

        # GHSA verified pattern: uses season and full suffix
        url = (
            f"{self.base_url}/{season}-ghsa-class-{class_lower}-"
            f"{gender_lower}-state-basketball-championship-bracket"
        )
    ...
    async def get_tournament_brackets(
        ...
        **ENUMERATION STRATEGY**:
        ...
        Args:
            ...
            classification: Specific classification (7A-1A), None for all
            gender: "Boys" or "Girls"
        ...
        self.logger.info(
            f"Fetching GHSA tournament brackets",
            year=year,
        ...
        self.logger.info(
            f"Fetched all GHSA tournament brackets",
            year=year,
    ...
    def _parse_bracket_html(
        ...
        GHSA bracket pages typically contain:
    ...
    def _create_game(
        ...
        team1_id = canonical_team_id("ghsa", team1)
        team2_id = canonical_team_id("ghsa", team2)

        return Game(
            game_id=f"ghsa_{year}_{classification.lower()}_{team1_id.split('_', 1)[1]}_vs_{team2_id.split('_', 1)[1]}",
    ...
    async def _parse_html_data(self, html: str, season: str) -> Dict:
        year = self._extract_year(season)
        soup = parse_html(html)
        bracket_data = self._parse_bracket_html(soup, year, "7A", "Boys", "")
        return {
    ...
    async def get_player(self, player_id: str) -> Optional[Player]:
        self.logger.warning("GHSA does not provide player data - use MaxPreps for Georgia")
        return None
