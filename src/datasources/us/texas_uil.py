"""
UIL (University Interscholastic League) DataSource Adapter - Texas

Provides authoritative tournament brackets, seeds, scores, and championship results
for Texas high school basketball. This is the SECOND LARGEST state system in the US.

**Data Authority**: UIL is the source of truth for:
...
**Base URL**: https://www.uiltexas.org

**URL Pattern**:
```
Basketball Brackets: /athletics/basketball/
Boys: /athletics/basketball/boys/
Girls: /athletics/basketball/girls/
Brackets: /brackets/{year}/
```

**Coverage**:
- Classifications: 6A, 5A, 4A, 3A, 2A, 1A (enrollment-based divisions)
- 1,400+ member schools (second largest in US after California)
- Boys and Girls tournaments
- All regions of Texas

**Texas Basketball Context**:
- Second-largest state system in US (1,400+ schools)
- Strong basketball tradition (LaMarcus Aldridge, Kevin Durant, T.J. Ford)
- UIL manages all high school athletics in Texas
- Enrollment-based classifications (6A largest schools, 1A smallest)
- Regional tournaments → state championships

**Special Features**:
- **Enrollment-based divisions** (unlike CIF-SS competitive equity)
- Regional tournament structure (UIL divides state into regions)
- Historical bracket data available
- State championship location: San Antonio (typically)
...
**Recommended Use**: UIL provides official tournament brackets.
...
class TexasUILDataSource(AssociationAdapterBase):
    """
    University Interscholastic League (UIL) Texas adapter.

    **PRIMARY PURPOSE**: Authoritative tournament bracket and postseason results.

    **SECOND LARGEST**: 1,400+ schools - only California is larger.

    This adapter provides:
    1. Tournament brackets for all classifications (6A-1A)
    ...
    **ARCHITECTURE**:
    ...
    - Enumerates classifications: 6A, 5A, 4A, 3A, 2A, 1A (enrollment-based)
    - Generates unique game IDs: uil_tx:{year}:{class}:{home}@{away}
    ...
    **LIMITATIONS**:
    ...
    source_type = DataSourceType.UIL
    source_name = "UIL Texas"
    base_url = "https://www.uiltexas.org"
    region = DataSourceRegion.US_TX

    # UIL specific constants
    # Enrollment-based classifications (6A = largest, 1A = smallest)
    ...
    def __init__(self):
        """Initialize UIL datasource with Texas-specific configuration."""
        super().__init__()
        self.settings = get_settings()
        self.logger.info(
            "UIL Texas initialized",
    ...
    def _build_bracket_url(
        ...
        """
        Build URL for UIL Texas state tournament bracket.

        **UIL URL Pattern** (verified 2024-11-13):
        ```
        /basketball/state-bracket/{season}-{classification}-{gender}-basketball-state-results

        Example:
        /basketball/state-bracket/2023-2024-6a-boys-basketball-state-results
        /basketball/state-bracket/2023-2024-5a-girls-basketball-state-results
        ```

        Note: UIL uses season format (2023-2024) not just year (2024).

        Args:
            classification: Classification name (6A, 5A, 4A, 3A, 2A, 1A)
            gender: "Boys" or "Girls"
            year: Tournament year (end year of season)
        ...
        prev_year = year - 1
        season = f"{prev_year}-{year}"  # e.g., "2023-2024"
        gender_lower = gender.lower()
        class_lower = classification.lower()

        # UIL verified pattern: uses season string and full suffix
        url = (
            f"{self.base_url}/basketball/state-bracket/"
            f"{season}-{class_lower}-{gender_lower}-basketball-state-results"
        )
    ...
    async def get_tournament_brackets(
        ...
        **ENUMERATION STRATEGY**:
        ...
        Args:
            ...
            classification: Specific classification (6A, 5A, etc.), None for all
            gender: "Boys" or "Girls"
        ...
        self.logger.info(
            f"Fetching UIL tournament brackets",
            year=year,
        ...
        self.logger.info(
            f"Fetched all UIL tournament brackets",
            year=year,
    ...
    def _parse_bracket_html(
        ...
        UIL bracket pages typically contain:
        ...
        Args:
            ...
            classification: Classification name (6A-1A)
            gender: Boys or Girls
            url: Source URL
    ...
    def _create_game(
        ...
        team1_id = canonical_team_id("uil_tx", team1)
        team2_id = canonical_team_id("uil_tx", team2)

        return Game(
            game_id=f"uil_tx_{year}_{classification.lower()}_{team1_id.split('_', 2)[2]}_vs_{team2_id.split('_', 2)[2]}",
    ...
    async def get_player(self, player_id: str) -> Optional[Player]:
        self.logger.warning("UIL does not provide player data - use MaxPreps for Texas")
        return None
