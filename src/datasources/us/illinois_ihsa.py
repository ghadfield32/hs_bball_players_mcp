"""
IHSA (Illinois State Association) DataSource Adapter

Provides authoritative tournament brackets, seeds, scores, and championship results
for Illinois high school basketball.

**Data Authority**: IHSA is the source of truth for:
- Tournament brackets (4 classifications: 1A, 2A, 3A, 4A)
- Seeds and matchups
...
**Coverage**:
- Classifications: 1A, 2A, 3A, 4A (enrollment-based)
- 800+ member schools
...
- 800+ schools
- Notable tradition (Dwyane Wade,Isiah Thomas,Anthony Davis)
- IHSA manages all high school athletics in Illinois
- Enrollment-based classifications (1A highest, 4A lowest)
...
**Recommended Use**: IHSA provides official tournament brackets.
...
class IllinoisIHSADataSource(AssociationAdapterBase):
    """
    Illinois IHSA adapter.

    **PRIMARY PURPOSE**: Authoritative tournament bracket and postseason results.

    **COVERAGE**: 800+ schools with basketball programs.

    This adapter provides:
    1. Tournament brackets for all classifications (1A, 2A, 3A, 4A)
    ...
    **ARCHITECTURE**:
    ...
    - Enumerates classifications: 1A, 2A, 3A, 4A
    - Generates unique game IDs: ihsa_{year}_{class}_{home}@{away}
    ...
    **LIMITATIONS**:
    ...
    source_type = DataSourceType.IHSA
    source_name = "IHSA"
    base_url = "https://www.ihsa.org"
    region = DataSourceRegion.US_IL

    # IHSA specific constants
    ...
    def __init__(self):
        """Initialize IHSA datasource with Illinois-specific configuration."""
        super().__init__()
    ...
    def _build_bracket_url(
        ...
        **IHSA URL Format**:
        ...
        /sports/basketball/boys/brackets/2025/1a/
        /sports/basketball/girls/brackets/2025/1a/
        ...
        Args:
            classification: Classification name (1A, 2A, 3A, 4A)
        ...
        # Try multiple URL patterns (IHSA structure may vary)
    ...
    async def get_tournament_brackets(
        ...
        **ENUMERATION STRATEGY**:
        ...
        Args:
            ...
            classification: Specific classification (1A, 2A, 3A, 4A), None for all
            gender: "Boys" or "Girls"
        ...
        self.logger.info(
            f"Fetching IHSA tournament brackets",
            year=year,
        ...
        self.logger.info(
            f"Fetched all IHSA tournament brackets",
            year=year,
    ...
    def _parse_bracket_html(
        ...
        IHSA bracket pages typically contain:
    ...
    def _create_game(
        ...
        team1_id = canonical_team_id("ihsa", team1)
        team2_id = canonical_team_id("ihsa", team2)

        return Game(
            game_id=f"ihsa_{year}_{classification.lower()}_{team1_id.split('_', 1)[1]}_vs_{team2_id.split('_', 1)[1]}",
    ...
    async def _parse_html_data(self, html: str, season: str) -> Dict:
        year = self._extract_year(season)
        soup = parse_html(html)
        bracket_data = self._parse_bracket_html(soup, year, "1A", "Boys", "")
        return {
    ...
    async def get_player(self, player_id: str) -> Optional[Player]:
        self.logger.warning("IHSA does not provide player data - use MaxPreps for Illinois")
        return None
