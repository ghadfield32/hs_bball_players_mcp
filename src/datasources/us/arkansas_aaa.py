"""
AAA (Arkansas State Association) DataSource Adapter

Provides authoritative tournament brackets, seeds, scores, and championship results
for Arkansas high school basketball.

**Data Authority**: AAA is the source of truth for:
...
- All regions of Arkansas

**Arkansas Basketball Context**:
- 250+ schools
- Strong basketball tradition
- AAA manages all high school athletics in Arkansas
...
**Recommended Use**: AAA provides official tournament brackets.
...
class ArkansasAAADataSource(AssociationAdapterBase):
    """
    Arkansas AAA adapter.
    ...
    **ARCHITECTURE**:
    ...
    - Generates unique game IDs: aaa_{year}_{class}_{home}@{away}
    ...
    **LIMITATIONS**:
    ...
    def __init__(self):
        """Initialize AAA datasource with Arkansas-specific configuration."""
        super().__init__()
    ...
    def _build_bracket_url(
        ...
        **AAA URL Format**:
        ...
        # Try multiple URL patterns (AAA structure may vary)
    ...
    async def get_tournament_brackets(
        ...
        **ENUMERATION STRATEGY**:
        ...
        self.logger.info(
            f"Fetching AAA tournament brackets",
            year=year,
        ...
        self.logger.info(
            f"Fetched all AAA tournament brackets",
            year=year,
    ...
    def _parse_bracket_html(
        ...
        AAA bracket pages typically contain:
    ...
    def _create_game(
        ...
        team1_id = canonical_team_id("aaa", team1)
        team2_id = canonical_team_id("aaa", team2)

        return Game(
            game_id=f"aaa_{year}_{classification.lower()}_{team1_id.split('_', 1)[1]}_vs_{team2_id.split('_', 1)[1]}",
    ...
    async def get_player(self, player_id: str) -> Optional[Player]:
        self.logger.warning("AAA does not provide player data - use MaxPreps for Arkansas")
        return None
