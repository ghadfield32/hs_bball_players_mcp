"""
Tests for OTE (Overtime Elite) DataSource Adapter

Run these tests to validate the OTE adapter implementation.
...
Usage:
    ...
    # Run with output
    pytest tests/test_datasources/test_ote.py -v -s
...
from src.datasources.us.ote import OTEDataSource


@pytest.mark.asyncio
class TestOTEDataSource:
    """Test suite for Overtime Elite adapter."""

    @pytest.fixture
    async def datasource(self):
        """Create OTE datasource instance."""
        ds = OTEDataSource()
        yield ds
        await ds.close()
    ...
    async def test_search_players(self, datasource):
        ...
        assert player.player_id.startswith("ote_"), "Player ID should have correct prefix"
        assert player.data_source is not None, "Player should have data source metadata"

        # Log results for debugging
        print(f"\n✓ Found {len(players)} OTE players")
    ...
    async def test_search_players_with_team_filter(self, datasource):
        ...
        # Try filtering by OTE team
        players = await datasource.search_players(team="City", limit=5)

        if len(players) == 0:
            pytest.skip("No players from teams with 'City' - check team names")

        # All returned players should be from matching team
        for player in players:
            if player.team_name:
                assert "city" in player.team_name.lower(), f"Team {player.team_name} doesn't match filter"

        print(f"\n✓ Team filter working: Found {len(players)} players from teams with 'City'")
    ...
    async def test_get_player_season_stats(self, datasource):
        ...
        assert stats.league == "Overtime Elite", "League should be OTE"
...
async def test_ote_integration_with_aggregator():
    """Test OTE adapter integration with aggregation service."""
    from src.services.aggregator import StatsAggregator

    aggregator = StatsAggregator()

    # Search across all sources (should include OTE if working)
    players = await aggregator.search_players_all_sources(limit=5)

    # Check if any OTE players found
    ote_players = [p for p in players if p.player_id.startswith("ote_")]

    if len(ote_players) == 0:
        pytest.skip("No OTE players found in aggregated results")

    print(f"\n✓ Integration test: Found {len(ote_players)} OTE players via aggregator")
