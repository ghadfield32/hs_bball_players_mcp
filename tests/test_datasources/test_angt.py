"""
Tests for ANGT (Adidas Next Generation Tournament) DataSource Adapter

Run these tests to validate the ANGT adapter implementation.
...
Usage:
    ...
    # Run with output
    pytest tests/test_datasources/test_angt.py -v -s
...
@pytest.mark.asyncio
class TestANGTDataSource:
    """Test suite for ANGT adapter."""
    ...
    async def test_search_players(self, datasource):
        ...
        assert player.player_id.startswith("angt_"), "Player ID should have correct prefix"
        assert player.data_source is not None, "Player should have data source metadata"

        # Log results for debugging
        print(f"\n✓ Found {len(players)} ANGT players")
    ...
    async def test_search_players_with_team_filter(self, datasource):
        ...
        # Try filtering by European club
        players = await datasource.search_players(team="Madrid", limit=5)

        if len(players) == 0:
            pytest.skip("No players from teams with 'Madrid' - check club names")

        # All returned players should be from matching team
        for player in players:
            if player.team_name:
                assert "madrid" in player.team_name.lower(), f"Club {player.team_name} doesn't match filter"

        print(f"\n✓ Team filter working: Found {len(players)} players from clubs with 'Madrid'")
    ...
    async def test_get_player_season_stats(self, datasource):
        ...
        assert stats.league == "ANGT", "League should be ANGT"
    ...
    async def test_data_quality(self, datasource):
        ...
        print(f"  With club: {with_team} ({with_team/len(players)*100:.1f}%)")
...
async def test_angt_integration_with_aggregator():
    """Test ANGT adapter integration with aggregation service."""
    from src.services.aggregator import StatsAggregator

    aggregator = StatsAggregator()

    # Search across all sources (should include ANGT if working)
    players = await aggregator.search_players_all_sources(limit=5)

    # Check if any ANGT players found
    angt_players = [p for p in players if p.player_id.startswith("angt_")]

    if len(angt_players) == 0:
        pytest.skip("No ANGT players found in aggregated results")

    print(f"\n✓ Integration test: Found {len(angt_players)} ANGT players via aggregator")
