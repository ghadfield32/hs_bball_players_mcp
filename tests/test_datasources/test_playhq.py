"""
Tests for PlayHQ DataSource Adapter

Run these tests to validate the PlayHQ adapter implementation.
...
Usage:
    ...
    # Run with output
    pytest tests/test_datasources/test_playhq.py -v -s
...
from src.datasources.australia.playhq import PlayHQDataSource


@pytest.mark.asyncio
class TestPlayHQDataSource:
    """Test suite for PlayHQ adapter."""

    @pytest.fixture
    async def datasource(self):
        """Create PlayHQ datasource instance."""
        ds = PlayHQDataSource()
        yield ds
        await ds.close()
    ...
    async def test_search_players(self, datasource):
        ...
        assert player.player_id.startswith("playhq_"), "Player ID should have correct prefix"
        assert player.data_source is not None, "Player should have data source metadata"

        # Log results for debugging
        print(f"\n✓ Found {len(players)} PlayHQ players")
    ...
    async def test_search_players_with_team_filter(self, datasource):
        ...
        # Try filtering by Australian team
        players = await datasource.search_players(team="Bullets", limit=5)

        if len(players) == 0:
            pytest.skip("No players from teams with 'Bullets' - check team names")

        # All returned players should be from matching team
        for player in players:
            if player.team_name:
                assert "bullets" in player.team_name.lower(), f"Team {player.team_name} doesn't match filter"

        print(f"\n✓ Team filter working: Found {len(players)} players from teams with 'Bullets'")
    ...
    async def test_get_player_season_stats(self, datasource):
        ...
        assert stats.league == "PlayHQ", "League should be PlayHQ"
...
async def test_playhq_integration_with_aggregator():
    """Test PlayHQ adapter integration with aggregation service."""
    from src.services.aggregator import StatsAggregator

    aggregator = StatsAggregator()

    # Search across all sources (should include PlayHQ if working)
    players = await aggregator.search_players_all_sources(limit=5)

    # Check if any PlayHQ players found
    playhq_players = [p for p in players if p.player_id.startswith("playhq_")]

    if len(playhq_players) == 0:
        pytest.skip("No PlayHQ players found in aggregated results")

    print(f"\n✓ Integration test: Found {len(playhq_players)} PlayHQ players via aggregator")
