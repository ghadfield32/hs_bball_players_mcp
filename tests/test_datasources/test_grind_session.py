"""
Tests for Grind Session DataSource Adapter

Run these tests to validate the Grind Session adapter implementation.
...
Usage:
    ...
    # Run with output
    pytest tests/test_datasources/test_grind_session.py -v -s
...
from src.datasources.us.grind_session import GrindSessionDataSource


@pytest.mark.asyncio
class TestGrindSessionDataSource:
    """Test suite for Grind Session adapter."""

    @pytest.fixture
    async def datasource(self):
        """Create Grind Session datasource instance."""
        ds = GrindSessionDataSource()
        yield ds
        await ds.close()
    ...
    async def test_search_players(self, datasource):
        ...
        assert player.player_id.startswith("grindsession_"), "Player ID should have correct prefix"
        assert player.data_source is not None, "Player should have data source metadata"

        # Log results for debugging
        print(f"\n✓ Found {len(players)} Grind Session players")
    ...
    async def test_search_players_with_team_filter(self, datasource):
        ...
        players = await datasource.search_players(team="Academy", limit=5)

        if len(players) == 0:
            pytest.skip("No players from teams with 'Academy' - check team names")

        # All returned players should be from matching team
        for player in players:
            if player.team_name:
                assert "academy" in player.team_name.lower(), f"Team {player.team_name} doesn't match filter"

        print(f"\n✓ Team filter working: Found {len(players)} players from teams with 'Academy'")
    ...
    async def test_get_player_season_stats(self, datasource):
        ...
        assert stats.league == "Grind Session", "League should be Grind Session"
    ...
    async def test_data_quality(self, datasource):
        ...
        print(f"  With team: {with_team} ({with_team/len(players)*100:.1f}%)")
...
async def test_grind_session_integration_with_aggregator():
    """Test Grind Session adapter integration with aggregation service."""
    from src.services.aggregator import StatsAggregator

    aggregator = StatsAggregator()

    # Search across all sources (should include Grind Session if working)
    players = await aggregator.search_players_all_sources(limit=5)

    # Check if any Grind Session players found
    grind_session_players = [p for p in players if p.player_id.startswith("grindsession_")]

    if len(grind_session_players) == 0:
        pytest.skip("No Grind Session players found in aggregated results")

    print(f"\n✓ Integration test: Found {len(grind_session_players)} Grind Session players via aggregator")
