"""
Fetch SBLive State HS Basketball Stats

Scrapes player-level season statistics from SBLive state coverage using
the SBLive adapter and saves to DuckDB for dataset building.

Coverage:
- States: WA, OR, CA, AZ, ID, NV (Western US)
...
Usage:
    ...
    python scripts/fetch_sblive_stats.py --states WA,OR,CA --limit 100 --save-to-duckdb
...
class SBLiveDataFetcher:
    """
    Fetches SBLive player stats across multiple states and seasons.

    Handles pagination, retries, multi-state fetching, and DuckDB storage.
    """

    # SBLive covered states (Western US)
    SBLIVE_STATES = ['WA', 'OR', 'CA', 'AZ', 'ID', 'NV']
    ...
    def __init__(
        ...
        """
        Initialize SBLive data fetcher.

        Args:
            states: List of state codes to fetch (e.g., ['WA', 'CA'])
        ...
        """
        # Validate states
        invalid_states = [s for s in states if s not in self.SBLIVE_STATES]
        if invalid_states:
            raise ValueError(
                f"Invalid SBLive states: {invalid_states}. "
                f"Valid states: {self.SBLIVE_STATES}"
            )
        ...
        self.sblive_source = None
        self.duckdb_storage = None

        logger.info(
            f"SBLiveDataFetcher initialized: states={states}, seasons={seasons}, "
    ...
    async def initialize(self):
        ...
        self.sblive_source = SBLiveDataSource()
    ...
    async def cleanup(self):
        ...
        if self.sblive_source:
            await self.sblive_source.close()
    ...
    async def fetch_players_for_state(
        ...
        try:
            # SBLive's search_players returns Player objects from state leaderboards
            players = await self.sblive_source.search_players(
                state=state,
                season=season,
    ...
    async def fetch_season_stats_for_player(
        ...
        """
        try:
            stats = await self.sblive_source.get_player_season_stats(
                player_id=player_id,
                season=season,
                state=state
            )
    ...
    async def fetch_all_data(self) -> Dict:
        ...
        logger.info("=" * 80)
        logger.info(
            f"Starting SBLive data fetch: {len(self.states)} states, "
    ...
    async def run(self) -> Dict:
        """
        Run the full SBLive data fetch pipeline.

        Returns:
            Dictionary with fetch results and summary
        """
        logger.info(
            f"\nStarting SBLive data fetch pipeline\n"
        ...
        try:
            ...
            logger.info("\n✅ SBLive data fetch pipeline completed successfully!")
            return result
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Fetch SBLive state HS basketball player stats"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default='all',
        help='Comma-separated state codes (WA,OR,CA,AZ,ID,NV) or "all" (default: all)'
    )
    ...
    if args.states.lower() == 'all':
        states = SBLiveDataFetcher.SBLIVE_STATES
    else:
